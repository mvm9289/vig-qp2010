...
void Object::CalculaNormalsVertex()
{
  for(unsigned int i=0; i<faces.size(); ++i)
    faces[i].computeNormalVertex(vertices);
}

void Object::Render(bool smooth)
{
  ...
  for (unsigned int i = 0; i < n; i++)
  {
    glBegin(GL_POLYGON);
      ...
      GLfloat ambient[] = {material.ka.r, material.ka.g, material.ka.b};
      GLfloat diffuse[] = {material.kd.r, material.kd.g, material.kd.b};
      GLfloat specular[] = {material.ks.r, material.ks.g, material.ks.b};
      glMaterialfv(GL_FRONT, GL_AMBIENT, ambient);
      glMaterialfv(GL_FRONT, GL_DIFFUSE, diffuse);
      glMaterialfv(GL_FRONT, GL_SPECULAR, specular);
      glMaterialf(GL_FRONT, GL_SHININESS, material.shininess);
      
      if (!smooth) glNormal3f(faces[i].normal.x, faces[i].normal.y, faces[i].normal.z);
    
      unsigned int m = faces[i].vertices.size();
      for (unsigned j = 0; j < m; j++)
      {
        unsigned int k = faces[i].vertices[j];
        if (smooth) glNormal3f(vertices[k].normal.x, vertices[k].normal.y, vertices[k].normal.z);
        glVertex3f(vertices[k].coord.x, vertices[k].coord.y, vertices[k].coord.z);
      }
    glEnd();
  }
}
